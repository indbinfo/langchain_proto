...
def init_page(root_dir):
    st.set_page_config(page_title='BC카드 PoC',
...
async def main_ui():
    ...
    with open(result_path + 'result_20240324145108.txt',
              'r',
              encoding='utf-8') as f:
        code_return = f.read()
            ...
            if st.form_submit_button(label='검색하기'):
                if user_question:
                    with st.spinner('실행중입니다..'):
                        response = get_response(code_return, user_question,
                        ...
                        st.markdown("""
                                    <table class="info-table" ... </table><br>
                                    """,
                                    unsafe_allow_html=True)
